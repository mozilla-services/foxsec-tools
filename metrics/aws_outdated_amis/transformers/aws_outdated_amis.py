...
def handle_day_files(src_dir, dest_dir, day_str):
    for file in sorted(os.listdir(src_dir)):
        if os.path.isdir(src_dir + "/" + file):
            ...
            for acc_file in fnmatch.filter(
                sorted(os.listdir(src_dir + "/" + account)), "*-" + day_str + ".json"
            ):
                ...
                with open(src_dir + "/" + account + "/" + acc_file) as f:
                    ...
                    if not "results" in data:
                        print(
                            "No results in " + src_dir + "/" + account + "/" + acc_file
                        )
                        continue
                    ...
                    for res in data["results"]:
                        if (
                            res["test_name"]
                            == "test_ec2_instance_running_required_amis"
                            or res["test_name"] == "test_ec2_instance_on_acceptable_ami"
                        ):
                            resDict = {}
                            resDict["day"] = day_str

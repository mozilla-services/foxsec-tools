...
def get_rra_query():
    return (
        "SELECT 'Risk Management' AS section, 'Must have RRA' AS item, foxsec_metrics.metadata_services.service, "
...
def get_observatory_query():
    return (
        "SELECT 'Web Applications' AS section, 'A plus on Observatory' AS item, foxsec_metrics.metadata_urls.service, "
...
def get_github_query_branch_protection():
    return (
        "SELECT 'Development' AS section, 'Enforce branch protection' AS item, service, appCode, '' as site, 'global' as environment, "
...
def get_baseline_query(section, item, column):
    return (
        ...
        + "' AS section, '"
        + item
        + "' AS item, foxsec_metrics.metadata_urls.service, "
        + "foxsec_metrics.metadata_urls.appCode, "
        ...
        + "foxsec_metrics.baseline_details.rule = '"
        + column
        + "' and "
...
def run_raw_query(query):
    ...
    response = client.start_query_execution(
        QueryString=query, ResultConfiguration={"OutputLocation": tempdir,}
    )
...
def main():
    ...
    run_day_query(
        get_baseline_query("Web Applications", "Cookies httponly", "rule_10010")
    )
    run_day_query(
        get_baseline_query("Web Applications", "Cookies secure", "rule_10011")
    )
    ...
    # Security features
    run_day_query(
        get_baseline_query("Security Features", "Anti CSRF tokens", "rule_10202")
    )

    # Common issues
    run_day_query(
        get_baseline_query(
            "Web Applications", "Prevent reverse tabnabbing", "rule_10108"
        )
    )

...
def get_bugs(bugids):
    ...
    return requests.get(BIGZILLA_BASE_URL + 'bug?id=' + ",".join(map(str, bugids)), headers=headers)


def get_bugs_for_component(product, component):
    headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
        'X-BUGZILLA-API-KEY' : BUGZILLA_API_KEY
    }

    return requests.get(BIGZILLA_BASE_URL + 'bug?product=' + product + "&component=" + component, headers=headers)
...
def get_args():
    ...
    parser.add_argument('-f', '--service_file',
						help='optional service file')
...
def handle_service(service_json, dest_file):
	...
	for comp in service_json['bugzilla']:
		...
		if len(comp['product']) == 0:
			...
			continue
		bugs_resp = get_bugs_for_component(comp['product'], comp['component'])
		if bugs_resp.status_code == 200:
			...
			if len(bugs_json['bugs']) == 0:
				print(service + ':\tNo bugs found in ' + comp['product'] + '/' + comp['component'])
			else:
			...
			print(service + '\tFailed: ' + str(bugs_resp.status_code) + ' for ' + comp['product'] + '/' + comp['component'])

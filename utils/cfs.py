...
def upload_file(locfile, desttype, destloc, destname):
    ...
    if desttype == "s3":
        ...
        raise ValueError(
            "Destination type not supported, currently only s3 is supported."
        )
...
def get_args():
    ...
    parser.add_argument("op", type=check_op, help="operation - 'upload' or 'download'")

    parser.add_argument("-f", "--local-file", required=True, help="local file")

    parser.add_argument("-t", "--dest-type", default="s3", help="destination type")

    parser.add_argument("-l", "--dest-loc", required=True, help="destination location")

    parser.add_argument("-n", "--dest-name", required=True, help="destination name")
